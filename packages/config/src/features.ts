...
const FEATURE_RULES: Readonly<Record<FeatureName, FeatureRule>> = {
  ...
  stripeBilling: {
    kind: 'all',
    keys: ['STRIPE_SECRET_KEY', 'STRIPE_PRO_PRICE_ID', 'STRIPE_WEBHOOK_SECRET'],
  },
  ...
  orchestratorProductionAutoApprove: {
    kind: 'truthy',
    keys: ['ORCHESTRATOR_AUTO_APPROVE_PRODUCTION'],
  },
};
...
export function buildFeatureReport(values: ResolvedValues): readonly FeatureReportEntry[] {
  return FEATURE_NAMES.map((feature) => {
    ...
    const enabled = rule.kind === 'any' ? unset.length < rule.keys.length : unset.length === 0;
