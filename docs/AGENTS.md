...
## Multi-Agent System (MAS) Roster

This repository is driven by a runnable orchestrator (`npm run agents:run`) plus specialist agents under `agents/`. Each agent has a `SYSTEM.md` role file, typed inputs/outputs, and a handoff contract back to the orchestrator.

**Do not invent work outside these domains.** Marketing/GTM is documented in `docs/` but does not have a product agent — there is no marketing app to build in v0.

### Loop the orchestrator must run

1. **Plan** — load goal, config, and repo state; assign tasks to domain agents.
2. **Implement** — specialists produce artifacts (code, fixtures, checklists).
3. **Test** — QA agent runs `npm test` / targeted vitest files.
4. **If fail** — Bug Reporter files a structured bug in `.anyx/bugs/` → Bug Fixer patches → re-test (bounded retries).
5. **Deploy** — DevOps produces a deploy checklist and a dry-run CI job. Live deploy only when secrets exist.
6. **If blocked on secrets / APIs / wallets / OAuth** — emit a **Config Request** (never stall). User fills `.env.local` or `config/user.config.yaml` and re-runs.

### Agent roster

| ID | Domain | Owns | Tools | Handoff |
|----|--------|------|-------|---------|
| `orchestrator` | Tech lead | Task graph, retries, run reports | spawn agents, write `.anyx/runs/` | Final run report |
| `x402` | x402 protocol / payments adapter | Challenge parse, PAYMENT header, facilitator client, fee math | fixtures, core tests, quote/pay contracts | Protocol artifacts + failing vectors |
| `ai-integrations` | LLM routing / agent plugins | LangChain tool, MCP server, OpenAI function defs, AgentKit action | package stubs, integration tests | Integration artifacts |
| `product` | PRD / scope | v0 slice vs later phases, acceptance criteria | docs/PRD, low-hanging-fruit | Scoped task list (no extra features) |
| `backend` | API / Hono | `/v1/quote`, `/v1/pay`, receipts, tokens, health | Hono routes, in-memory/DB store | API artifacts |
| `frontend` | UX / dashboard | Config wizard, developer portal pages | HTML/UI, `/setup` | UI artifacts |
| `security` | Wallet / auth / keys | Hot signer, API keys, never-log-secrets, EIP-3009 | config schema, auth middleware | Threat notes + config gates |
| `qa` | Testing | Vitest, fixtures, coverage of core paths | `npm test`, bug filing | Pass/fail + logs |
| `devops` | CI / deploy | GitHub Actions, Docker, Fly dry-run | workflows, health checks | Deploy checklist |
| `bug-reporter` | Failures | Structured `BugReport` JSON | test logs, stack traces | Bug tickets |
| `bug-fixer` | Patches | Minimal fix for a filed bug | repo edit + re-test | Patch result |
| `config-broker` | Human-in-the-loop secrets | Missing API keys, RPCs, wallets, OAuth | `check-config`, Config Requests | Unblocks other agents |

### Handoff contract (all specialists)
...
{
  agentId: string
  status: 'ok' | 'failed' | 'blocked_config' | 'skipped'
  summary: string
  artifacts: string[]          // file paths
  bugs?: BugReport[]
  configRequests?: ConfigRequest[]
  next?: string[]              // agent IDs to run next
}
```

Bugs, config requests, and run reports are JSON with stable schemas in `agents/orchestrator/types.ts`. Do not invent parallel formats.

### How to run

```bash
npm run check-config          # plain-language missing keys
npm run agents:run            # default verify-v0 loop
npm run agents:run -- --goal pay-smoke
```
