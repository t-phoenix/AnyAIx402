...
export const LAUNCH_TASKS: readonly TaskDefinition[] = [
  {
    ...
    acceptanceCriteria: [
      ...
      { kind: 'command', description: 'All contract tests passing', command: 'forge test', requiresBinary: 'forge' },
      { kind: 'manual', description: 'Smart contracts audited, or a formal review completed' },
      { kind: 'manual', description: 'Contracts deployed to Base Sepolia and exercised end to end' },
      { kind: 'manual', description: 'Contracts deployed to Base mainnet' },
      { kind: 'command', description: 'API deployed to Fly.io and health check passing', command: 'curl -fsS https://api.anyx.xyz/health -o /dev/null' },
      { kind: 'command', description: 'SDK published to npm as @anyx/sdk@0.1.0', command: 'npm view @anyx/sdk@0.1.0 version' },
      { kind: 'command', description: 'Documentation site live with the quickstart', command: 'curl -fsS https://docs.anyx.xyz -o /dev/null' },
      { kind: 'command', description: 'llms.txt reachable', command: 'curl -fsS https://anyx.xyz/llms.txt -o /dev/null' },
      { kind: 'command', description: 'OpenAPI spec served', command: 'curl -fsS https://api.anyx.xyz/openapi.json -o /dev/null' },
      { kind: 'command', description: 'Swagger UI served', command: 'curl -fsS https://api.anyx.xyz/docs -o /dev/null' },
      ...
      { kind: 'manual', description: 'Rate limiting verified at 150 requests per minute on the free tier' },
