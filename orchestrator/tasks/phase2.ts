...
export const PHASE_2_TASKS: readonly TaskDefinition[] = [
  {
    ...
    acceptanceCriteria: [
      {
        kind: 'file-exists',
        description: 'Router contract exists',
        path: 'packages/contracts/src/AnyXRouter.sol',
      },
      {
        kind: 'file-exists',
        description: 'Swap executor exists',
        path: 'packages/contracts/src/SwapExecutor.sol',
      },
      {
        kind: 'file-exists',
        description: 'Fee collector exists',
        path: 'packages/contracts/src/FeeCollector.sol',
      },
      {
        kind: 'file-contains',
        description: 'Reentrancy protection applied',
        path: 'packages/contracts/src/AnyXRouter.sol',
        pattern: 'nonReentrant',
      },
      {
        kind: 'file-exists',
        description: 'Fork test exists',
        path: 'packages/contracts/test/AnyXRouter.t.sol',
      },
      {
        kind: 'command',
        description: 'Foundry tests pass',
  ...
  },
  {
    ...
    acceptanceCriteria: [
      {
        kind: 'file-exists',
        description: 'Deploy script exists',
        path: 'packages/contracts/script/Deploy.s.sol',
      },
      {
        kind: 'file-exists',
        description: 'Contracts README documents addresses',
        path: 'packages/contracts/README.md',
      },
      {
        kind: 'command',
        description: 'Deploy script compiles',
        command: 'forge build',
        requiresBinary: 'forge',
      },
      {
        kind: 'manual',
        description:
          'Testnet deployment executed and addresses recorded by a human with a funded deployer key',
      },
    ],
    verifyCommands: [
      { command: 'forge build', cwd: 'packages/contracts', requiresBinary: 'forge' },
